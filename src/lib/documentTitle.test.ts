...
describe('documentTitle with tab-scoped sessions', () => {
  ...
  it('names the signed-in user', () => {
    expect(scoped('Ada Owner')).toBe('RealTV Draft — Ada Owner')
  })

  it('is the bare app name with nobody signed in', () => {
    expect(scoped(null)).toBe('RealTV Draft')
    expect(scoped(undefined)).toBe('RealTV Draft')
  })

  it('does not leave a dangling separator for an empty name', () => {
    expect(scoped('')).toBe('RealTV Draft')
    expect(scoped('   ')).toBe('RealTV Draft')
  })

  it('trims a padded name rather than spacing it out', () => {
    expect(scoped('  Bob Member  ')).toBe('RealTV Draft — Bob Member')
  })
})
...
describe('documentTitle with one session per browser', () => {
  ...
  it('never names the user', () => {
    expect(documentTitle({ displayName: 'Ada Owner', tabScopedAuth: false })).toBe('RealTV Draft')
    expect(documentTitle({ displayName: null, tabScopedAuth: false })).toBe('RealTV Draft')
  })
})
