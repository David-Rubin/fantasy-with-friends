...
export const signUpUser = functions.https.onCall(async (data: { displayName: string; email: string; inviteCode?: string }) => {
...
export const validateInviteCode = functions.https.onCall(async (
  data: { code: string },
  context,
) => {
...
async function redeemInviteCode(
  ...
  batch.set(db.doc(`leagues/${leagueId}/members/${uid}`), {
    role: 'member',
    joinedAt: Date.now(),
  }, { merge: true })
...
async function recalcTeamTotals(seasonId: string) {
  ...
  const episodeDocs = await Promise.all(
    episodeScoresSnap.docs.map(async (epDoc) => {
      const scoresSnap = await db.collection(`seasons/${seasonId}/episodeScores/${epDoc.id}/contestantScores`).get()
      const scores: Record<string, ContestantScoreDoc> = {}
      scoresSnap.docs.forEach((d) => { scores[d.id] = d.data() as ContestantScoreDoc })
